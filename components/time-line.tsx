...
export const TimeLine = () => {
  return (
    <div className="flex flex-col justify-center divide-y divide-woodsmoke-200">
        ...
        <div className="-my-6">
          {dataAboutPage.map((data) => (
            <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
              ...
              >
                <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-xs font-semibold uppercase w-20 h-6 mb-3 sm:mb-0 text-secondary bg-woodsmoke-900 rounded-full">
                  {data.date}
                </time>
                <p className="text-xl font-bold text-woodsmoke-400">
                  {data.subtitle}
                </p>
              </div>
              <p className="text-woodsmoke-400 max-w-md">{data.description}</p>
            </div>
          ))}
        </div>
      </div>
